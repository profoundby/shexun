...
- (void)setupCellWithData:(NSDictionary *)data andImage:(NSString *)imageURL {
    
    // Pull out sample data
    NSString *nameString = data[@"title"];
    NSString *companyString = data[@"title"];
    NSString *bioString = data[@"content"];
    ...
    [self.sampleImageView sd_setImageWithURL:[NSURL URLWithString:imageURL]];
}

+ (CGSize)defaultCellSize {
    //return (CGSize){CGRectGetWidth([[UIScreen mainScreen] bounds]), 85};
    return (CGSize){320, 85};
}
